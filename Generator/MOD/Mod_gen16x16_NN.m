...
for dataset = 1
    out = [];
    path = strcat('/home/user12/Saad_External/MODCOD_DataSet/MOD_DATASET/MOD_16SampNN_',num2str(dataset),'.h5');
        ...
        for i= 1:25
            ...
            D_counter = 1;
                for sel_SNR = 1:length(SNR)
                    ...
                    %SD = 1/sqrt(2);
                    %H=(normrnd(0,SD,[1 102400])+1i*normrnd(0,SD,[1 102400]));
                    Rec = stream(512:length(stream)-513) + No_floor;

                    amp = abs(Rec);
                    pha = rem(phase(Rec),pi);
                    
                    

                    DataSet(1,:,D_counter:D_counter+399) = reshape(real(Rec),[256,1,400]);	%R_reshape;
                    DataSet(2,:,D_counter:D_counter+399) = reshape(imag(Rec),[256,1,400]);    %I_reshape;
